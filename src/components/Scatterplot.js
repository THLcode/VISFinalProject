...
/**
 * selectedFeatures: 2개 feature 정보 [{ category, feature, stat }, { ... }]
 * brushedPoints: brush로 선택된 데이터 배열
 * onBrush: brush 이벤트 발생 시 부모로 brushed 데이터 전달하는 콜백
 */
const ScatterPlot = ({ selectedFeatures, brushedPoints, onBrush }) => {
  const svgRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (selectedFeatures.length < 2) {
      return; // 2개 미만이면 그래프 없음
    }
    const xFeature = selectedFeatures[0];
    const yFeature = selectedFeatures[1];

    const students = Object.values(data);
    const plotData = students
      .map((student) => {
        const xVal =
